...
$ git log
```
Выводится список коммитов c описанием следующего вида:  
```bash
$ git log

commit 5d3e42e7e0ad4822d3cd06504112ad592c34a7b4
Author: Artem <antonov.artem2000@gmail.com>
Date:   Mon Feb 26 23:00:36 2024 +0300

    Readme create
```


**Первая** строка содержит hash-код коммита.  
**Вторая** строка содержит информацию об авторе коммита (имя и почта).  
**Третья** строка содержит информацию о времени коммита.  
**Четвертая** строка содержит сообщение о коммите.  


Можно вызвать и короткую версию лога:
```bash
$ git log --oneline

6518e54 Update README.md
5d3e42e Readme create
```
Сокращенная строка содержит описание коммита и сокращенный хэш, который обладает той же функциональностью, что и полный.

--- 
## Файл HEAD
---
**Head** -  один из служебных файлов папки .git. Он указывает на коммит, который сделан последним (то есть на самый новый).  
Содержание файла - ссылка на служебный файл, который хранит хэш последнего коммита.
```bash
$ cat HEAD # команда cat показывает содержимое файла
ref: refs/heads/master # в файле вот такая ссылка 
```

Head можно отправлять в качестве аргумента функций, Git поймет, что вы имеете в виду последний коммит.

---
## Состояние файла в репозитории
---
Файл может находиться в следующих состояниях:
1. **Untraked** - файл находится в репозитории, но гит его не отслеживает (нет сохраненных или заявленных на коммит версий)
2. **Tracked** - файл отслеживается гитом
3. **Staged** - файл заявлен на коммит и будет сохранен при следующем коммите
4. **Modified** - traked-файл в котором были произведены незакоммиченные изменения


На следующей диаграмме приведены почти все возможные смены состояния в файлах:
```mermaid
graph LR;
  untracked -- "git add" --> tracked/staged;
  tracked/staged -- "git commit" --> tracked/comitted;
  tracked/staged -- "изменения в файле" --> tracked/staged/modified;
  tracked/staged/modified -- "git commit" --> tracked/comitted;
  tracked/staged/modified -- "git add" --> tracked/staged;
  tracked/comitted -- "изменения в файле" --> tracked/modified;
  tracked/modified -- "git add" --> tracked/staged;
```
